...
- **歌单系统**:智能歌单(常听 / 收藏 / 专辑 / 歌手 / 按格式 / 假无损 / 疑似 AI)与自定义歌单(新建 / 重命名 / 删除 / 批量添加歌曲 / 拖拽排序 / 快捷切换),JSON 持久化
...
- **曲库分析**:长按首页播放列表按钮打开分析面板,展示曲库音频格式占比圆环,并对疑似假无损 FLAC(基于频谱分析的升频 / 砖墙转码检测)与疑似 AI 生成歌曲(立体声相关性、高频缺口、谐波梳等多征象启发式)做增量校验,逐曲结果持久化缓存;点击任一格式或识别类目即可切换到对应智能歌单
- **无损升级**:对疑似假无损的本地曲目,点击音源格式条在线搜索原曲(来源可切换),确认后下载无损版本替换本地文件
- **歌单同步**:粘贴平台分享链接(网易云 / QQ / 酷我)拉取远程歌单,本地同名跳过、其余按最高可用音质下载入库并自动创建歌单(网易云内置解析,其余平台需代理音源)
...
| 页面 | 内容 |
| --- | --- |
| 首页 | 权限引导对话框(全部授权后自动关闭)、沉浸式播放器(旋转碟片封面 + 封面取色渐变背景)、5 行同步歌词(字号与行数可调)、可刷新的播放列表、收藏、定时关闭、横屏模式、右滑在线搜索(五平台切换 + 音质选择)与左滑歌单面板、上下滑动切歌(长按封面 / 标题可刷新封面、歌词及重命名;长按播放列表按钮打开曲库分析;点击音源格式条可对可疑曲目无损升级) |
...
│       │   ├── data/                    # 数据层
│       │   │   ├── music/               #   曲库扫描 / 播放状态存储 / 在线音源 / 代理音源
│       │   │   │   ├── api/             #     在线音乐源(网易云 / QQ / 酷狗 / 酷我 / 咪咕)
│       │   │   │   ├── metadata/        #     封面管理与元数据读写
│       │   │   │   ├── model/           #     曲目数据模型
│       │   │   │   └── proxy/           #     代理音源(导入 / 解析 / 引擎 / 存储 / 歌单同步)
│       │   │   ├── playlist/            #   自定义歌单实体与存储
...
│       │   ├── domain/music/            # 领域层(播放状态 / 下载 / 频谱分析 / 工具)
│       │   ├── floatingwindow/          # 悬浮窗与迷你播放器(视图管理 + 权限流程)
│       │   │   ├── mini_player/         #   迷你播放器悬浮条与视图管理
│       │   │   └── music_panel/         #   完整音乐面板视图管理
│       │   ├── log/                     # CrashLogManager
│       │   ├── navigation/              # Navigation3 类型安全路由
│       │   ├── screens/                 # 页面(首页 / 设置 / 排版)
│       │   │   ├── home/                #   首页播放器(紧凑 / 展开分区 + 共享组件)
│       │   │   │   ├── compact/         #     竖屏播放分区
│       │   │   │   ├── expanded/        #     横屏播放分区
...
被两个及以上功能复用的代码上提至顶层(`data/`、`theme/`、`utils/`、`ui/`),仅单页使用的代码保留在页面模块内。播放领域逻辑位于 `domain/music`(状态 / 下载 / 频谱分析 / 工具),底层由 `data/music` 层支撑,并通过窗口级 `MusicPanelStateHolder` 暴露给 UI;悬浮 UI(完整面板 + 迷你播放器)位于 `floatingwindow/`(视图管理,按 `music_panel/` 与 `mini_player/` 拆分),可组合项在 `ui/music/`,实际播放由 `service/MusicPlaybackService`(Media3 ExoPlayer + `MediaSessionService`)驱动。

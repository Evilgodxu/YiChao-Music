package com.yichao.evilgodxu.theme

import androidx.compose.ui.graphics.Color

// 浅色主题配色
val md_theme_light_primary = Color(0xFF3F51B5)
val md_theme_light_onPrimary = Color(0xFFFFFFFF)
val md_theme_light_primaryContainer = Color(0xFFE8EAF6)
val md_theme_light_onPrimaryContainer = Color(0xFF1A237E)

val md_theme_light_secondary = Color(0xFF656D76)
val md_theme_light_onSecondary = Color(0xFFFFFFFF)
val md_theme_light_secondaryContainer = Color(0xFFE0E0E0)
val md_theme_light_onSecondaryContainer = Color(0xFF1A1A1A)

val md_theme_light_tertiary = Color(0xFF8250DF)
val md_theme_light_onTertiary = Color(0xFFFFFFFF)
val md_theme_light_tertiaryContainer = Color(0xFFF3E8FF)
val md_theme_light_onTertiaryContainer = Color(0xFF3B1E6E)

val md_theme_light_background = Color(0xFFF6F8FA)
val md_theme_light_onBackground = Color(0xFF24292F)
val md_theme_light_surface = Color(0xFFFFFFFF)
val md_theme_light_onSurface = Color(0xFF1F2328)
val md_theme_light_surfaceVariant = Color(0xFFF3F4F6)
val md_theme_light_onSurfaceVariant = Color(0xFF656D76)

val md_theme_light_outline = Color(0xFFD0D7DE)
val md_theme_light_outlineVariant = Color(0xFFD0D7DE)
val md_theme_light_scrim = Color(0xFF000000)

val md_theme_light_error = Color(0xFFCF222E)
val md_theme_light_onError = Color(0xFFFFFFFF)
val md_theme_light_errorContainer = Color(0xFFF9DEDC)
val md_theme_light_onErrorContainer = Color(0xFF410E0B)

val md_theme_light_inverseSurface = Color(0xFF30363D)
val md_theme_light_inverseOnSurface = Color(0xFFF0F6FC)
val md_theme_light_inversePrimary = Color(0xFFB1B8DF)

val md_theme_light_surfaceTint = Color(0xFF3F51B5)
val md_theme_light_surfaceDim = Color(0xFFE8ECF0)
val md_theme_light_surfaceBright = Color(0xFFFFFFFF)
val md_theme_light_surfaceContainerLowest = Color(0xFFFFFFFF)
val md_theme_light_surfaceContainerLow = Color(0xFFF6F8FA)
val md_theme_light_surfaceContainer = Color(0xFFEFF1F5)
val md_theme_light_surfaceContainerHigh = Color(0xFFE8ECF0)
val md_theme_light_surfaceContainerHighest = Color(0xFFE2E5EA)

// 深色主题配色
val md_theme_dark_primary = Color(0xFFB1B8DF)
val md_theme_dark_onPrimary = Color(0xFF1A237E)
val md_theme_dark_primaryContainer = Color(0xFF3F51B5)
val md_theme_dark_onPrimaryContainer = Color(0xFFE8EAF6)

val md_theme_dark_secondary = Color(0xFF8B949E)
val md_theme_dark_onSecondary = Color(0xFF1A1A1A)
val md_theme_dark_secondaryContainer = Color(0xFF5C5C5C)
val md_theme_dark_onSecondaryContainer = Color(0xFFE0E0E0)

val md_theme_dark_tertiary = Color(0xFFA371F7)
val md_theme_dark_onTertiary = Color(0xFFFFFFFF)
val md_theme_dark_tertiaryContainer = Color(0xFF5C3B8C)
val md_theme_dark_onTertiaryContainer = Color(0xFFF3E8FF)

val md_theme_dark_background = Color(0xFF0D1117)
val md_theme_dark_onBackground = Color(0xFFC9D1D9)
val md_theme_dark_surface = Color(0xFF161B22)
val md_theme_dark_onSurface = Color(0xFFF0F6FC)
val md_theme_dark_surfaceVariant = Color(0xFF21262D)
val md_theme_dark_onSurfaceVariant = Color(0xFF8B949E)

val md_theme_dark_outline = Color(0xFF30363D)
val md_theme_dark_outlineVariant = Color(0xFF21262D)
val md_theme_dark_scrim = Color(0xFF000000)

val md_theme_dark_error = Color(0xFFF85149)
val md_theme_dark_onError = Color(0xFFFFFFFF)
val md_theme_dark_errorContainer = Color(0xFF8C1D18)
val md_theme_dark_onErrorContainer = Color(0xFFF9DEDC)

val md_theme_dark_inverseSurface = Color(0xFFF0F6FC)
val md_theme_dark_inverseOnSurface = Color(0xFF1F2328)
val md_theme_dark_inversePrimary = Color(0xFF3F51B5)

val md_theme_dark_surfaceTint = Color(0xFFB1B8DF)
val md_theme_dark_surfaceDim = Color(0xFF0D1117)
val md_theme_dark_surfaceBright = Color(0xFF2D323B)
val md_theme_dark_surfaceContainerLowest = Color(0xFF090D13)
val md_theme_dark_surfaceContainerLow = Color(0xFF161B22)
val md_theme_dark_surfaceContainer = Color(0xFF1A2028)
val md_theme_dark_surfaceContainerHigh = Color(0xFF252B33)
val md_theme_dark_surfaceContainerHighest = Color(0xFF30363D)
